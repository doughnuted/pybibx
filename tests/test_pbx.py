...
class TestPbxProbeExtended(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            cls.pbx.topic_model = BERTopic(verbose=False, embedding_model="all-MiniLM-L6-v2") # Using a fast model
            ...
            cls.pbx.network_co_authorship(topn_authors=5, report=False) # Generate a small graph
    ...
    def test_00_initialization_and_prerequisites(self):
        ...
        if self.pbx: # Proceed only if pbx object was created
            self.assertTrue(hasattr(self.pbx, 'topic_model'), "BERTopic model should be initialized.")
            self.assertIsNotNone(self.pbx.topics_df, "Topics DataFrame should not be None after fit_transform.")
            self.assertIsInstance(self.pbx.topics_df, pd.DataFrame, "topics_df should be a DataFrame.")
    ...
    def test_01_dynamic_topic_modeling(self):
        ...
        if not self.pbx or not hasattr(self.pbx, 'topic_model'):
            self.skipTest("Skipping dynamic topic modeling test due to setup failure or no topic model.")

        timestamps = pd.to_datetime(self.pbx.data['year'].replace('UNKNOWN', pd.NaT)).dt.year.fillna(0).astype(int).tolist()
        # Ensure timestamps and docs have the same length
        docs_for_dynamic = [str(text) if pd.notna(text) else "" for text in self.pbx.data['abstract'].tolist()]
        ...
        try:
            ...
            self.assertIsNotNone(self.pbx.topics_over_time_df, "topics_over_time_df should not be None.")
            self.assertIsInstance(self.pbx.topics_over_time_df, pd.DataFrame, "topics_over_time_df should be a DataFrame.")
    ...
    def test_02_hierarchical_topic_modeling(self):
        ...
        if not self.pbx or not hasattr(self.pbx, 'topic_model'):
            self.skipTest("Skipping hierarchical topic modeling test due to setup failure or no topic model.")
        try:
            self.pbx.topics_hierarchical()
            self.assertIsNotNone(self.pbx.hierarchical_topics_df, "hierarchical_topics_df should not be None.")
            self.assertIsInstance(self.pbx.hierarchical_topics_df, pd.DataFrame, "hierarchical_topics_df should be a DataFrame.")

            fig = self.pbx.graph_topics_hierarchical()
            self.assertIsNotNone(fig, "graph_topics_hierarchical should return a figure.")
    ...
    def test_03_louvain_community_detection(self):
        ...
        if not self.pbx or not hasattr(self, 'sample_graph') or self.sample_graph is None:
            self.skipTest("Skipping Louvain community test due to setup failure or no sample graph.")
        ...
        if len(self.sample_graph.nodes) < 2 or self.sample_graph.number_of_edges() == 0:
            ...
            G_test = nx.Graph()
            G_test.add_edges_from([(0,1),(0,2),(0,3),(1,2),(1,4),(2,3),(3,4),(4,5)])
            self.sample_graph = G_test

        try:
            communities = self.pbx.network_community_louvain(graph_obj=self.sample_graph)
            self.assertIsNotNone(self.pbx.louvain_communities, "louvain_communities should not be None.")
            self.assertIsInstance(self.pbx.louvain_communities, list, "louvain_communities should be a list.")
            if communities: # if communities are found
                 self.assertIsInstance(communities[0], set, "Each community should be a set of nodes.")

            fig = self.pbx.graph_network_community_louvain(graph_obj=self.sample_graph, communities=self.pbx.louvain_communities)
            self.assertIsNotNone(fig, "graph_network_community_louvain should return a figure.")
    ...
    def test_04_network_metrics_calculation(self):
        ...
        if not self.pbx or not hasattr(self, 'sample_graph') or self.sample_graph is None:
            self.skipTest("Skipping network metrics test due to setup failure or no sample graph.")
        ...
        try:
            metrics = self.pbx.network_calculate_metrics(graph_obj=self.sample_graph)
            self.assertIsNotNone(self.pbx.network_metrics, "network_metrics should not be None.")
            self.assertIsInstance(self.pbx.network_metrics, dict, "network_metrics should be a dictionary.")

            expected_keys = ['degree_centrality', 'betweenness_centrality', 'closeness_centrality', 'eigenvector_centrality', 'clustering_coefficient', 'graph_density']
            for key in expected_keys:
                self.assertIn(key, self.pbx.network_metrics, f"{key} should be in network_metrics.")
            ...
            if isinstance(e, nx.NetworkXError) and "Eigenvector centrality" in str(e):
                print(f"Skipping eigenvector part of network metrics due to graph structure: {e}")
