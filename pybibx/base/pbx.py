...
class pbx_probe:
    ...
    def __read_bib(self, bib, db="scopus", del_duplicated=True):
        ...
        def get_corresponding_authors_and_affiliations(corr_address):
            if not isinstance(corr_address, str):
                return [], []
            match = re.search(
                r"Corresponding Author\s+([^;]+);([^;]+)", corr_address, re.IGNORECASE
            )
            if not match:
                return [], []
        ...
        def map_authors_to_affiliations(row):
            ...
            if isinstance(affiliations_str, str):
                ...
                affiliations = []
            ca_authors, ca_affiliations = get_corresponding_authors_and_affiliations(
            ...
            for ca_aff in ca_affiliations:
                affiliations = [
                    aff for aff in affiliations if ca_aff.lower() not in aff.lower()
                ]
        ...
        if db == "scopus" and file_extension == ".csv":
            ...
            if (
                "abbrev_source_title" not in data.columns
                and "abbreviated source title" in data.columns
            ):
                data.rename(
                    columns={"abbreviated source title": "abbrev_source_title"},
                    inplace=True,
                )
            ...
            if (
                "author_keywords" not in data.columns
                and "author keywords" in data.columns
            ):
                data.rename(
                    columns={"author keywords": "author_keywords"}, inplace=True
                )
            ...
            if (
                "correspondence_address" not in data.columns
                and "correspondence address" in data.columns
            ):
                data.rename(
                    columns={"correspondence address": "correspondence_address"},
                    inplace=True,
                )
            if "editor" not in data.columns and "editors" in data.columns:
                data.rename(columns={"editors": "editor"}, inplace=True)
            if (
                "funding_details" not in data.columns
                and "funding details" in data.columns
            ):
                data.rename(
                    columns={"funding details": "funding_details"}, inplace=True
                )
            ...
            if (
                "language" not in data.columns
                and "language of original document" in data.columns
            ):
                data.rename(
                    columns={"language of original document": "language"}, inplace=True
                )
            ...
            if db == "pubmed":
                ...
                for i in range(0, len(f_list)):
                    if i == 0 and f_list[i][:6] != "      ":
                        ...
                        i > 0
                        and f_list[i][:6] != "      "
                        and f_list[i][:6] != f_list[i - 1][:6]
                        and (
                            f_list[i][:6].lower() != "fau - "
                            and f_list[i][:6].lower() != "au  - "
                            and f_list[i][:6].lower() != "auid- "
                            and f_list[i][:6].lower() != "ad  - "
                        )
                    ):
                        f_list_.append(f_list[i])
                    elif (
            ...
            for i in range(0, len(f_list)):
                if f_list[i].find("@") == 0 or f_list[i][:4].lower() == "pmid":
                    ...
                    if db == "wos":
                        ...
                        rhs.append("WoS")
                    doc = doc + 1
                if (f_list[i].find("=") != -1 and f_list[i].find(" ") != 0) or (
                    f_list[i].find("=") != -1 and f_list[i].find("=") == 15
                ):  # DBLP
        ...
        # WoS -> Scopus
        data["document_type"] = data["document_type"].replace(
        ...
        data["author_keywords"] = data["author_keywords"].apply(
            lambda x: x.replace(",", ";")
        )
        if db == "wos":
        ...
        if "affiliation" in data.columns and "affiliation_" in data.columns:
            filtered_indices = data[
                (data["affiliation"] == "UNKNOWN") & (data["affiliation_"] != "UNKNOWN")
            ].index
            ...
            for i in filtered_indices:
                ...
                new_parts = [
                    part.split(",", 1)[-1].strip() for part in parts[1:] if "," in part
                ]
        ...
        if "affiliation" in data.columns and "affiliations" in data.columns:
            filtered_indices = data[
                (data["affiliation"] == "UNKNOWN") & (data["affiliations"] != "UNKNOWN")
            ].index
            filtered_indices = list(filtered_indices)
            for i in filtered_indices:
        ...
        if db == "pubmed":
            data["affiliation"] = data.apply(
                lambda row: assign_authors_to_affiliations(
                    row["author"], row["affiliation"]
                ),
                axis=1,
            )
        if db == "wos":
            data["affiliation_"] = data["affiliation_"].str.replace(
                r"(?<=[A-Z])\.", "#", regex=True
            )
            data["affiliation_"] = (
                data["affiliation_"]
    ...
    def __get_past_citations_year(self):
        df = (
            self.data[["author", "title", "doi", "year", "references"]]
            .sort_values(by="year")
    ...
    def __get_countries(self):
                ...
                if self.database.lower() == "wos":
                    if "Corresponding Author" in row["affiliation_"]:
                        ...
                        if (
                            ...
                            and corresponding_author in self.author_country_map
                        ):
                            self.corr_a_country_map[corresponding_author] = (
                                self.author_country_map[corresponding_author]
                            )
                else:
                    corresponding_author = next(
                        (
                            ...
                            in row["correspondence_address1"].lower()
                        ),
                        None,
                    )
                    if (
            ...
            for index, row in self.data.iterrows():
                if self.aut[index]:
                    ...
                    if first_author in self.author_country_map:
                        self.frst_a_country_map[first_author] = self.author_country_map[
                            first_author
                        ]
            return ctr, u_ctr
        ...
        data["processed_affiliation"] = data.apply(
            preprocess_affiliation, axis=1
        ).str.lower()
        ...
        data["processed_affiliation"] = data["processed_affiliation"].replace(
            "united states of america", "usa", regex=True
        )
        data["processed_affiliation"] = data["processed_affiliation"].replace(
            "united states", "usa", regex=True
        )
        data["processed_affiliation"] = data["processed_affiliation"].replace(
            country_replacements, regex=True
        )
            ...
            for author in authors:
                ...
                for aff in affiliations:
                    ...
                    if detected_country != "UNKNOWN":
                        break
                if not any(
                    entry[0] == index for entry in self.author_country_map[author]
                ):
    ...
    def replace_unknowns(self, list_of_lists):
        ...
        for sublist in list_of_lists:
            ...
            for item in sublist:
                if item == "UNKNOWN":
                    new_sublist.append(
                        prev_value if prev_value is not None else "UNKNOWN"
                    )
                else:
    ...
    def __get_institutions(self):
        ...
        def extract_top_institution_with_priority(text, institution_names):
            ...
            for segment in segments:
                ...
                matches = [
                    (part.strip(), self.inst_priority.get(keyword, 0))
                    for part in sub_parts
                    for keyword in self.inst_priority.keys()
                    if keyword in part
                ]
                if matches:
        ...
        affiliations = (
            self.data["affiliation"].fillna("").str.lower()
            if "affiliation" in self.data.columns
            else pd.Series([""] * len(self.data))
        )
        affiliations_wos = (
            self.data["affiliation_"].fillna("").str.lower()
            if "affiliation_" in self.data.columns
            else pd.Series([""] * len(self.data))
        )
        affiliations_wos = (
            self.data["affiliation_"].fillna("").str.lower()
        ...
        top_institutions = processed_affiliations.apply(
            lambda row: extract_top_institution_with_priority(
                row, self.institution_names
            )
        )
        inst = [top for top in top_institutions]
        flattened_institutions = [
            institution for sublist in top_institutions for institution in sublist
        ]
                ...
                for institution in inst[index]:
                    for _, aff in enumerate(processed_affiliations[index].split(";")):
                        if author in aff and institution in aff:
                            self.author_inst_map[author].append(
                                (index, re.sub(r"^(?:[A-Za-z]\.\s?)+", "", institution))
                            )
                if len(self.author_inst_map[author]) == 0:
                    self.author_inst_map[author].append((index, "UNKNOWN"))
        self.author_inst_map = {
        ...
        for index, row in self.data.iterrows():
            if self.database == "wos":
                if "corresponding author" in row["affiliation_"].lower():
                    ...
                    if (
                        corresponding_author
                        and corresponding_author in self.author_inst_map
                    ):
                        self.corr_a_inst_map[corresponding_author] = (
        ...
        for index, row in self.data.iterrows():
            if self.aut[index]:
                ...
                if first_author in self.author_inst_map:
                    self.frst_a_inst_map[first_author] = self.author_inst_map[
                        first_author
                    ]
        return inst, u_inst
    ...
    def __get_collaboration_year(self):
        ...
        years.append("Total")
        dy_collab_year = pd.DataFrame(
            np.zeros((len(years), len(n_collaborators))),
            index=years,
            columns=n_collaborators,
        )
        ...
        for i in range(0, dy_collab_year.shape[0]):
            ci = sum(
                [
                    (j + 1) * dy_collab_year.iloc[i, j]
                    for j in range(0, dy_collab_year.shape[1] - 1)
                ]
            )
    ...
    def __get_ref_year(self):
        ...
        for ref in self.u_ref:
            ...
            valid_years = [
                int(year) for year in matches if 1665 <= int(year) <= date_end
            ]  # The oldest scientific journal is Philosophical Transactions, which was launched in 1665 by Henry Oldenburg
    ...
    def __get_ref_id(self):
        ...
        keys = np.where(
            sources.isin(["scopus", "pubmed"]),
            keys_1,
            np.where(sources == "wos", keys_2, None),
        )
    ...
    def word_cloud_plot(
        ...
        if len(rmv_custom_words) > 0:
            ...
            text = [
                x.replace(" ", "")
                for x in text
                if x.replace(" ", "") not in rmv_custom_words
            ]
            corpora = " ".join(text)
        wordcloud = WordCloud(
    ...
    def get_top_ngrams(
        ...
        fig = go.Figure(
            go.Bar(
                ...
                orientation="h",
                marker=dict(
                    color="rgba(246, 78, 139, 0.6)", line=dict(color="black", width=1)
                ),
            ),
        )
    ...
    def tree_map(self, view="browser", entry="kwp", topn=20):
        ...
        fig.update_layout(
            title=title, margin=dict(l=10, r=10, t=40, b=10), paper_bgcolor="white"
        )
        fig.show()
        return
    ...
    def authors_productivity(self, view="browser", topn=20):
        ...
        productivity = pd.DataFrame(
            np.zeros((topn, len(years))), index=key, columns=years
        )
        Xv = []
        Yv = []
        Xe = []
        ...
        self.ask_gpt_ap = self.ask_gpt_ap.loc[
            (self.ask_gpt_ap.sum(axis=1) != 0), (self.ask_gpt_ap.sum(axis=0) != 0)
        ]
        node_list_a = [
            str(int(productivity.iloc[Xv[i], Yv[i]])) for i in range(0, len(Xv))
        ]
            ...
            if int(year) in years:
                ...
                for country in countries:
                    if country != "unknown":
                        ...
                        yearly_data[int(year)][country] = (
                            yearly_data[int(year)][country] + 1
                        )
        all_years_data = {}
    ...
    def institution_productivity(self, view="browser", topn=20):
        ...
        self.ask_gpt_ip = self.ask_gpt_ip.loc[
            (self.ask_gpt_ip.sum(axis=1) != 0), (self.ask_gpt_ip.sum(axis=0) != 0)
        ]
        node_list_a = [
            str(int(productivity.iloc[Xv[i], Yv[i]])) for i in range(0, len(Xv))
        ]
    ...
    def source_productivity(self, view="browser", topn=20):
        ...
        self.ask_gpt_sp = self.ask_gpt_sp.loc[
            (self.ask_gpt_sp.sum(axis=1) != 0), (self.ask_gpt_sp.sum(axis=0) != 0)
        ]
        node_list_a = [
            str(int(productivity.iloc[Xv[i], Yv[i]])) for i in range(0, len(Xv))
        ]
    ...
    def plot_evolution_year(
        ...
        y_idx = [
            i
            for i in range(0, self.data.shape[0])
            if int(self.data.loc[i, "year"]) >= start
            and int(self.data.loc[i, "year"]) <= end
        ]
        if len(rmv_custom_words) == 0:
        ...
        for j in range(dict_y[start], dict_y[end] + 1):
            ...
            if len(theme_vec) > 0:
                ...
                n_val = [
                    names[i] + " (" + str(int(values[i])) + ")"
                    for i in range(0, len(names))
                ]
                self.ask_gpt_ep = (
                    self.ask_gpt_ep + " " + str(years[j]) + ": " + ", ".join(n_val)
                )
                data = go.Bar(
                    x=[years[j]] * len(values),
                    y=values,
                    text=names,
                    hoverinfo="text",
                    textangle=0,
        ...
        layout = go.Layout(
            ...
            margin=dict(b=10, l=5, r=5, t=10),
            plot_bgcolor="#f5f5f5",
            xaxis=dict(tickangle=35, showticklabels=True, type="category"),
        )
    ...
    def plot_bars(self, view="browser", statistic="dpy", topn=20):
        ...
        if statistic.lower() == "dpy":
            ...
            c2 = int(value[-1] * (2 / 3))
            key = [
                key[i] for i in range(0, len(key)) if value[i] > c1 and value[i] <= c2
            ]
            value = [
                value[i]
        ...
        if statistic.lower() in ["dpy", "cpy", "ppy", "ltk"]:
            ...
            if statistic.lower() != "ltk":
                ...
                fig.add_hline(
                    y=avg_line, line_color="red", line_width=1, line_dash="solid"
                )
            fig.update_layout(
                title=title,
                xaxis_title=x_lbl,
                    ...
                    if rmv_unknowns:
                        ...
                        if a_unknown and b_unknown:
                            updated_pairs.append(
                                (f"UNKNOWN_{source_col}", f"UNKNOWN_{target_col}")
                            )
                        elif a_unknown:
    ...
    def plot_heatmap_y_per_x(
        ...
        for doc_id, row in sk_data.iterrows():
            ...
            for xi in xs:
                for yi in ys:
                    if rmv_unknowns and (
                        str(xi).lower() == "unknown" or str(yi).lower() == "unknown"
                    ):
                        continue
        ...
        matrix_ids = [
            [list(set(cell)) if isinstance(cell, list) else cell for cell in row]
            for row in matrix_ids
        ]
        matrix_counts = [
            [len(cell) if isinstance(cell, list) else 0 for cell in row]
            for row in matrix_ids
        ]
    ...
    def sankey_diagram(
        self,
        view="browser",
        entry=["aut", "cout", "inst", "jou", "kwa", "kwp", "lan"],
        rmv_unknowns=False,
        topn=None,
    ):
        # ----------------------------------------------------------------------
        ...
        def hierarchical_filtering(sk_data, entry, topn, rmv_unknowns):
            ...
            if len(topn) != len(entry) - 1:
                raise ValueError(
                    f"topn must have length {len(entry) - 1}, received {len(topn)}."
                )
            ...
            filtered_relationships[(entry[0], entry[1])] = Counter(
                [(a, b) for a in prev_top_items for b in prev_level_data[a]]
            )
                ...
                for source, targets in prev_level_data.items():
                    for target in targets:
                        possible_next_targets = [
                            new_target
                            for (x, new_target) in current_relationships
                            if x == target
                        ]
                        next_target_counts = Counter(possible_next_targets)
                        sorted_next_targets = sorted(
                            next_target_counts.items(), key=lambda x: x[1], reverse=True
                        )
                        top_targets = [
                            b
                            for b, _ in sorted_next_targets[
                                : topn[min(i, len(topn) - 1)]
                            ]
                        ]
                        filtered_current_level.extend(
                            [(target, new_target) for new_target in top_targets]
                        )
                filtered_relationships[(source_col, target_col)] = Counter(
                    filtered_current_level
                )
                prev_level_data = {}
        ...
        all_relationships, relationships = hierarchical_filtering(
            sk_data, entry, topn, rmv_unknowns
        )
    ...
    def __total_and_self_citations(self):
        ...
        for researcher in self.u_aut:
            ...
            if not authored_papers:
                ...
                continue
            total_citations = sum(
                self.citation[paper_idx] for paper_idx in authored_papers
            )
            researcher_lower = researcher.lower()
            self_citations = sum(
                1
                for paper_idx in authored_papers
                for ref in preprocessed_refs[paper_idx]
                if researcher_lower in ref
            )
        ...
        if len(stop_words) > 0:
            ...
            for i in range(0, len(corpus)):
                ...
                text = [
                    x.replace(" ", "")
                    for x in text
                    if x.replace(" ", "") not in sw_full
                ]
                corpus[i] = " ".join(text)
                if verbose:
                    print(
                        "Removing Stopwords: " + str(i + 1) + " of " + str(len(corpus))
                    )
            if verbose:
    ...
    def docs_projection(
        ...
        if len(custom_projection) == 0 and not embeddings:
            transformed = decomposition.fit_transform(dtm)
        elif len(custom_projection) == 0 and embeddings:
            transformed = decomposition.fit_transform(embds)
        elif (
            custom_projection.shape[0] == self.data.shape[0]
            and custom_projection.shape[1] >= 2
        ):
        ...
        for i in range(0, n):
            ...
            n_trace.append(
                go.Scatter(
                    ...
                    mode="markers+text",
                    marker=dict(
                        symbol="circle-dot", size=node_size, color=self.color_names[i]
                    ),
                    text=node_list,
                    hoverinfo="text",
                    hovertext=n_id,
    ...
    def ref_citation_matrix(self, tgt_ref_id=[], date_start=None, date_end=None):
        ...
        for article_idx, pub_year in enumerate(self.dy):
            year = int(pub_year)
            if (date_start is not None and year < date_start) or (
                date_end is not None and year > date_end
            ):
                continue
                ...
                citation_records.append(
                    {
                        "Reference": ref_name,
                        "Reference ID": ref_id,
                        "Citing Articles": (article_idx, year),
                    }
                )
        ...
        result_df = (
            ...
            .agg(
                {
                    "Reference ID": lambda x: x.iloc[0],
                    "Citing Articles": lambda x: list(set(x)),
                }
            )
            .reset_index()
        )
    ...
    def plot_top_refs(
        ...
        self.top_refs = pd.DataFrame(
            {
                "Reference": labels_n,
                "Reference ID": labels_i,
                "Reference Year": years_top_refs,
                "Raw Citation Counts": values,
            }
        )
        fig = go.Figure(
            data=[
                go.Pie(
                    ...
                    marker=dict(
                        colors=self.color_names, line=dict(color="black", width=1)
                    ),
                )
            ]
        )
    ...
    def plot_citation_trajectory(self, view="browser", ref_names=[], ref_ids=[]):
            ...
            for r in article_refs:
                ...
                if r in selected_refs:
                    if pub_year in citation_trajectory[r]:
                        citation_trajectory[r][pub_year] = (
                            citation_trajectory[r][pub_year] + 1
                        )
        fig = go.Figure()
        for idx, ref in enumerate(selected_refs):
            x_values = [year for year in x_range if citation_trajectory[ref][year] != 0]
            y_values = [
                citation_trajectory[ref][year]
                for year in x_range
                if citation_trajectory[ref][year] != 0
            ]
        ...
        fig.update_layout(
            title=dict(
                text="Citation Trajectory Analysis",
    ...
    def plot_rpys(self, view="browser", peaks_only=False):
        ...
        self.rpys_rs = pd.DataFrame(
            {
                "Years": years,
                "Raw Citation Counts": counts,
                "Smoothed Citation Counts": smoothed_counts,
            }
        )
        bar_colors = [
        ...
        fig.add_trace(
            go.Bar(
                x=years, y=counts, name="Raw Citation Counts", marker_color=bar_colors
            )
        )
        fig.add_trace(
            go.Scatter(
    ...
    def plot_co_citation_network(self, view="browser", target_ref_id="", topn=10):
        if view == "browser":
            pio.renderers.default = "browser"
        citing_articles = [
            idx for idx, refs in enumerate(self.ref_id) if target_ref_id in refs
        ]
        co_cited_refs = []
        ...
        self.top_co_c = pd.DataFrame(
            ref_details, columns=["Reference ID", "Reference", "Year", "Count"]
        )
        G = nx.Graph()
        G.add_node(
            target_ref_id,
        ...
        node_trace = go.Scatter(
            ...
            text=node_text,
            marker=dict(
                size=node_size, color=node_color, line=dict(width=1.5, color="black")
            ),
            textposition="top center",
            hoverinfo="text",
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                showgrid=False, zeroline=False, showticklabels=False, scaleanchor="y"
            ),
            yaxis=dict(
    ...
    def salsa(self, max_iter=150, tol=1e-6, topn_decade=5):
        ...
        def build_hubs_authorities(
            ...
            for it in range(0, max_iter):
                ...
                factor1 = np.divide(
                    hubs, out_degrees, out=np.zeros_like(hubs), where=(out_degrees != 0)
                )
                new_auth = np.dot(A.T, factor1)
                factor2 = np.divide(
                    new_auth,
                    in_degrees,
                    out=np.zeros_like(new_auth),
                    where=(in_degrees != 0),
                )
                ...
                if (
                    np.abs(hubs - old_hubs).sum() < tol
                    and np.abs(authorities - old_auth).sum() < tol
                ):
                    break
        ...
        hubs, authorities = build_hubs_authorities(
            A, hubs, authorities, out_degrees, in_degrees, max_iter, tol
        )
        year_aggregates = {}
        ...
        for decade in decades:
            indices = [
                idx
                for idx, year in enumerate(result["node_years"])
                if year != -1 and int(year // 10 * 10) == decade
            ]
            sorted_indices = sorted(
                indices, key=lambda i: result["authorities"][i], reverse=True
            )
            top_by_decade_a[decade] = [
                (result["node_ids"][i], result["authorities"][i])
                for i in sorted_indices[:topn]
            ]
            sorted_indices = sorted(
                indices, key=lambda i: result["hubs"][i], reverse=True
            )
            top_by_decade_h[decade] = [
                (result["node_ids"][i], result["hubs"][i])
                for i in sorted_indices[:topn]
            ]
    ...
    def detect_sleeping_beauties(self, topn=10, min_count=10):
        ...
        for ref, counts_dict in citation_trajectory.items():
            ...
            metrics[ref] = {
                "B": B,
                "SBI": B / denom if denom > 0 else 0.0,
                "t_a": t_a,
                "c0": c0,
                "cm": cm,
                "t_m": t_m,
    ...
    def detect_princes(self, metrics):
        ...
        for _, row in metrics.iterrows():
            sb_id = row["ref"]
            citing_indices = [
                idx for idx, refs in enumerate(self.ref_id) if sb_id in refs
            ]
            sb_citers = []
            for idx in citing_indices:
                ...
                sb_citers.append(
                    {
                        "id": self.table_id_doc["ID"][idx],
                        "pub_year": int(pub_year),
                        "references": self.ref_id[idx],
                    }
                )
                ...
                results.append(
                    {
                        "SB_id": sb_id,
                        "PR_id": prince_id,
                        "PR_pub_year": prince["pub_year"],
                        "co_citation_count": co_citation_counts[prince_id],
                    }
                )
            else:
                results.append(
    ...
    def __adjacency_matrix_aut(self, min_colab=1):
        ...
        self.matrix_a = pd.DataFrame.sparse.from_spmatrix(
            adjacency, index=tgt_entry_u, columns=tgt_entry_u
        )
    ...
    def __adjacency_matrix_ref(self, min_cites=2, local_nodes=False):
        ...
        self.matrix_r = pd.DataFrame.sparse.from_spmatrix(
            sparse_matrix, columns=self.u_ref
        )
    ...
    def network_collab(
        ...
        fig, axes = plt.subplots(
            rows, cols, figsize=(cols * 5, rows * 5), facecolor="white"
        )
        ...
        for idx, target in enumerate(targets):
            ...
            labels = {
                node: "" if G.nodes[node]["color"] == nd_a else node
                for node in G.nodes()
            }
    ...
    def network_sim(
        ...
        if node_labels:
            ...
            mode = "markers"
            size = 10
        if node_labels and node_size > 0:
            mode = "markers+text"
            size = node_size
        elif not node_labels and node_size > 0:
            mode = "markers"
            size = node_size
        ...
        for name in u_rows:
            ...
            n_id = (
                self.data.loc[int(name), "author"]
                + " ("
                + self.data.loc[int(name), "year"]
                + "). "
                + self.data.loc[int(name), "title"]
                + ". "
                + self.data.loc[int(name), "journal"]
                + ". doi:"
                + self.data.loc[int(name), "doi"]
                + ". "
            )
        ...
        self.sim_table = pd.DataFrame(
            np.zeros((len(edges), 2)), columns=["Pair Node", "Sim(" + sim_type + ")"]
        )
        self.ask_gpt_sim = pd.DataFrame(
            np.zeros((len(edges), 3)),
            columns=["Node 1", "Node 2", "Simimilarity (" + sim + ") Between Nodes"],
        )
        ...
        nids_list_s = [
            "id: " + node_list_s[i] + "<br>" + nids_list_s[i]
            for i in range(0, len(nids_list_s))
        ]
        ...
        fig_s.update_traces(
            textfont_size=10, textfont_color="blue", textposition="top center"
        )
        fig_s.show()
        return
    ...
    def network_adj_map(self, view="browser", connections=True, country_lst=[]):
        if view == "browser":
            pio.renderers.default = "browser"
        lat_ = [
            self.country_lat_long[i][0]
            for i in range(0, len(self.country_lat_long))
            if self.country_names[i] in self.u_ctr
        ]
        lon_ = [
            self.country_lat_long[i][1]
            for i in range(0, len(self.country_lat_long))
            if self.country_names[i] in self.u_ctr
        ]
        iso_3 = [
            self.country_alpha_3[i]
            for i in range(0, len(self.country_lat_long))
            if self.country_names[i] in self.u_ctr
        ]
        ...
        fig_cm.update_geos(
            ...
            landcolor="#f0f0f0",
            showocean=True,
            oceancolor="#7fcdff",  # '#def3f6', '#7fcdff',
            showlakes=False,
            lakecolor="blue",
            showrivers=False,
            rivercolor="blue",
    ...
    def network_adj_dir(
        ...
        if node_labels and node_size == -1:
            mode = "markers+text"
            size = 50
        elif not node_labels and node_size == -1:
            mode = "markers"
            size = 10
        elif node_labels and node_size > 0:
            mode = "markers+text"
            size = node_size
        elif not node_labels and node_size > 0:
            mode = "markers"
        ...
        color = [
            G.nodes[n]["color"] if len(G.nodes[n]) > 0 else "black" for n in G.nodes()
        ]
        ...
        self.nids_list = [
            "<br>".join(textwrap.wrap(txt, width=50)) for txt in self.nids_list
        ]
        self.nids_list = [
            "id: " + self.node_list[i] + "<br>" + self.nids_list[i]
            for i in range(0, len(self.nids_list))
        ]
    ...
    def network_adj(
        ...
        if centrality == "degree":
            ...
            self.table_centr.insert(
                0,
                "Name",
                [
                    dict_[self.table_centr.iloc[i, 0]]
                    for i in range(0, self.table_centr.shape[0])
                ],
            )
            ...
            color = [value[n] for n in self.H.nodes()]
            self.table_centr = pd.DataFrame(
                value.items(), columns=["Node", "Betweenness"]
            )
            self.table_centr = self.table_centr.sort_values(
            ...
            color = [value[n] for n in self.H.nodes()]
            self.table_centr = pd.DataFrame(
                value.items(), columns=["Node", "Closeness"]
            )
            self.table_centr = self.table_centr.sort_values(
            ...
            color = [value[n] for n in self.H.nodes()]
            self.table_centr = pd.DataFrame(
                value.items(), columns=["Node", "Eigenvector"]
            )
            self.table_centr = self.table_centr.sort_values(
            ...
            for com in community_list:
                community_list.index(com)
                for node in com:
                    self.H.nodes[node]["color"] = self.color_names[
                        community_list.index(com)
                    ]
        ...
        self.fig_a.update_layout(
            yaxis=dict(scaleanchor="x", scaleratio=0.5),
            plot_bgcolor="rgb(255, 255, 255)",
            hoverlabel=dict(font_size=12),
        )
        self.fig_a.update_traces(
            textfont_size=font_size, textfont_color="blue", textposition="top center"
        )
        self.fig_a.show()
        if label_type != "id":
            if adj_type == "aut":
    ...
    def find_nodes_dir(
        self, view="browser", article_ids=[], ref_ids=[], node_size=-1, font_size=10
    ):
        if view == "browser":
            pio.renderers.default = "browser"
        if node_size > 0:
        ...
        if len(article_ids) > 0 or len(ref_ids) > 0:
            if len(article_ids) > 0:
                ...
                color_ids.extend(
                    ["red" if node.find("r_") >= 0 else "blue" for node in article_ids]
                )
                text_ids.extend([node for node in article_ids])
                hover_ids.extend(
                    [self.nids_list[self.node_list.index(node)] for node in article_ids]
                )
                for ids in article_ids:
                    edge_list_ai.extend(
                        [item for item in self.edge_list if ids == item[0]]
                    )
                    node_pair = [
                        self.edge_list[i][1]
                        for i in range(0, len(self.edge_list))
                        if ids in self.edge_list[i]
                    ]
                    idx_ids.extend([self.node_list.index(node) for node in node_pair])
                    color_ids.extend(
                        [
                            "red" if node.find("r_") >= 0 else "blue"
                    ...
                    hover_ids.extend(
                        [
                            self.nids_list[self.node_list.index(node)]
                            for node in node_pair
                        ]
                    )
                xa = []
            ...
            if len(ref_ids) > 0:
                ...
                color_ids.extend(
                    ["red" if node.find("r_") >= 0 else "blue" for node in ref_ids]
                )
                text_ids.extend([node for node in ref_ids])
                hover_ids.extend(
                    [self.nids_list[self.node_list.index(node)] for node in ref_ids]
                )
                for ids in ref_ids:
                    edge_list_ri.extend(
                        [item for item in self.edge_list if ids == item[1]]
                    )
                    node_pair = [
                        self.edge_list[i][0]
                        for i in range(0, len(self.edge_list))
    ...
    def find_nodes(
        ...
        fig_.update_traces(
            mode="markers",
            line=dict(width=0),
            marker=dict(color="rgba(0, 0, 0, 0.1)", size=7),
        )
        if len(node_ids) > 0:
            edge_list_ai = []
            idx_ids = []
            ...
            hover_ids.extend(
                [self.nids_list_a[self.node_list_a.index(node)] for node in node_ids]
            )
            if not node_only:
                for ids in node_ids:
                    edge_list_ai.extend(
                        [
                            item
                            for item in self.edge_list_a
                            if ids in item and item not in edge_list_ai
                        ]
                    )
                    node_pair = [
                        self.edge_list_a[i][1]
                        for i in range(0, len(self.edge_list_a))
                        if ids in self.edge_list_a[i] and self.edge_list_a[i][1] != ids
                    ]
                    node_pair.extend(
                        [
                            self.edge_list_a[i][0]
                            for i in range(0, len(self.edge_list_a))
                            if ids in self.edge_list_a[i]
                            and self.edge_list_a[i][0] != ids
                        ]
                    )
                    idx_ids.extend([self.node_list_a.index(node) for node in node_pair])
                    color_ids.extend(
                        ["#e0cc92" for n in node_pair]
                    )  # self.H.nodes[n]['color']
                    text_ids.extend([node for node in node_pair])
                    hover_ids.extend(
                        [
                            self.nids_list_a[self.node_list_a.index(node)]
                            for node in node_pair
                        ]
                    )
                xa = []
                ...
                for i in range(0, len(xa), 2):
                    fig_.add_trace(
                        go.Scatter(
                            ...
                            mode="lines",
                            line=dict(
                                color="rgba(0, 0, 0, 0.25)", width=0.5, dash="solid"
                            ),
                            hoverinfo="none",
                            name="",
                        )
    ...
    def network_hist(
        ...
        def filter_matrix_by_citations(matrix_r, min_links):
            ...
            for node, link_count in row_sums_sorted.items():
                if link_count >= min_links:
                    ...
                    linked_nodes = filtered_matrix.loc[node][
                        filtered_matrix.loc[node] > 0
                    ].index
        ...
        fig.show()
        self.ask_gpt_hist = pd.DataFrame(
            citations, columns=["Paper ID", "Reference ID"]
        )
        self.ask_gpt_hist["Paper ID"] = self.ask_gpt_hist["Paper ID"].astype(str)
        self.ask_gpt_hist = self.ask_gpt_hist.merge(
        ...
        self.ask_gpt_hist["Reference ID"] = self.ask_gpt_hist["Reference ID"].astype(
            str
        )
        self.ask_gpt_hist = self.ask_gpt_hist.merge(
        ...
        self.ask_gpt_hist = self.ask_gpt_hist[
            self.ask_gpt_hist["Reference_Year"] <= self.ask_gpt_hist["Paper_Year"]
        ]
        condition = (
            self.ask_gpt_hist["Reference_Year"] == self.ask_gpt_hist["Paper_Year"]
        )
        ...
        self.ask_gpt_hist["Paper ID (Year)"] = self.ask_gpt_hist.apply(
            lambda row: f"{row['Paper ID']} ({row['Paper_Year']})", axis=1
        )
        self.ask_gpt_hist["Reference ID (Year)"] = self.ask_gpt_hist.apply(
    ...
    def analyze_hist_citations(self, citations, min_path_size=2):
        ...
        for path in recent_to_old_paths:
            if not any(
                set(path).issubset(set(existing_path))
                for existing_path in filtered_paths
            ):
                filtered_paths.append(path)
        recent_to_old_paths = filtered_paths
        print(
            "Most Referenced Paper ID:",
            most_referenced_paper[0],
            "-> Cited",
            most_referenced_paper[1],
            "Times",
        )
        print(
            "Paper ID that Cites the Most:",
            most_citing_paper[0],
            "-> Cites",
            most_citing_paper[1],
            "Papers",
    ...
    def topics_representatives(self):
        ...
        self.df_rep = pd.DataFrame(
            np.zeros((self.topic_info.shape[0], 2)), columns=["Topic", "Docs"]
        )
    ...
    def graph_topics(self, view="browser"):
        ...
        topics_label = [
            "Topic "
            + str(self.topic_info.iloc[i, 0])
            + " ( Count = "
            + str(self.topic_info.iloc[i, 1])
            + ") "
            for i in range(0, self.topic_info.shape[0])
        ]
        column = 1
        ...
        for i in range(0, self.topic_info.shape[0]):
            ...
            trace = go.Bar(
                ...
                orientation="h",
                marker=dict(
                    color=self.color_names[i], line=dict(color="black", width=1)
                ),
            )
    ...
    def graph_topics_distribution(self, view="browser"):
        ...
        fig = go.Figure(
            go.Bar(
                ...
                hovertext=words,
                marker=dict(
                    color="rgba(78, 246, 215, 0.6)", line=dict(color="black", width=1)
                ),
                name="",
            ),
        )
        ...
        fig.update_layout(
            paper_bgcolor="rgb(189, 189, 189)", plot_bgcolor="rgb(189, 189, 189)"
        )
        fig.show()
        return
    ...
    def graph_topics_projection(self, view="browser", method="tsvd"):
        ...
        x_range = (
            transformed[:, 0].min() - abs((transformed[:, 0].min()) * 0.35),
            transformed[:, 0].max() + abs((transformed[:, 0].max()) * 0.35),
        )
        y_range = (
            transformed[:, 1].min() - abs((transformed[:, 1].min()) * 0.35),
            transformed[:, 1].max() + abs((transformed[:, 1].max()) * 0.35),
        )
    ...
    def graph_topics_time(self, view="browser"):
        ...
        topics_over_time = self.topic_model.topics_over_time(
            abstract, year, nr_bins=self.date_end - self.date_str
        )
    ...
    def topics_words(self, doc_id=0):
        ...
        df = self.topic_model.visualize_approximate_distribution(
            abstracts[doc_id], token[doc_id]
        )
        df = df.data.T
        df.columns = [
            f"Topic {col.split('_')[0]}" if col.split("_")[0].isdigit() else col
    ...
    def topics_authors(self, topn=15):
        ...
        doc_id["Author"] = doc_id["Document"].apply(
            lambda x: [author for author in targets if author in x]
        )
        doc_id = doc_id[doc_id["Author"].apply(len) > 0]
        unique_topics = sorted(set(topic for topic in self.topics))
        summary = pd.DataFrame(index=targets, columns=unique_topics + ["Total"]).fillna(
            0
        )
    ...
    def plot_word_embeddings(
        ...
        for i, (pos, neg) in enumerate(zip(positive, negative)):
            ...
            try:
                similar_words = model.wv.most_similar(
                    positive=pos, negative=neg, topn=topn
                )
        ...
        fig.update_layout(
            ...
            legend=dict(
                title="Queries", font=dict(size=font_size), itemsizing="constant"
            ),
        )
        fig.show()
        return
    ...
    def summarize_abst_peg(
        ...
        if len(corpus) > 0:
            ...
            tokens = tokenizer.encode(
                corpus, return_tensors="pt", max_length=max_L, truncation=True
            )
            summary = pegasus.generate(
                tokens,
                min_length=min_L,
                max_length=max_L,
                length_penalty=2.0,
    ...
    def summarize_abst_chatgpt(
        ...
        if len(corpus) > 0:
            ...
            if not join_articles:
                ...
                corpus = " ".join(corpus)
                prompt = query + ":\n\n" + f"{i + 1}. {corpus}\n"
            summary = self.query_chatgpt(
                prompt, model, max_tokens, n, temperature, flag, api_key
            )
        else:
            summary = "No abstracts were found in the selected set of documents"
        return summary
    ...
    def ask_chatgpt_ap(
        ...
        char_limit=4097,
        api_key="your_api_key_here",
        query="give me insights about the following information, related to authors productivity by year",
        model="text-davinci-003",
        max_tokens=2000,
        n=1,
        temperature=0.8,
        ...
        for element, row in df.iterrows():
            ...
            paper_counts = ", ".join(
                [
                    f"({year}: {count} paper{'s' if count > 1 else ''})"
    ...
    def ask_chatgpt_col_an(
        self,
        char_limit=4097,
        api_key="your_api_key_here",
        query="give me insights about the following network information, knowing that Node 1 is connected with Node 2",
        model="text-davinci-003",
        max_tokens=2000,
        n=1,
        temperature=0.8,
    ...
    def ask_chatgpt_ep(
        ...
        char_limit=4097,
        api_key="your_api_key_here",
        query="give me insights about the following information, related to words apperance by year",
        model="text-davinci-003",
        max_tokens=2000,
        n=1,
        temperature=0.8,
    ...
    # Function: Ask chatGPT about Citation Analysis
    def ask_chatgpt_hist(
        self,
        char_limit=4097,
        api_key="your_api_key_here",
        query="give me insights about the following information relating the most influential references, also discover if there is relevant network connections",
        model="text-davinci-003",
        max_tokens=2000,
        n=1,
        temperature=0.8,
        ...
        for i in range(0, self.ask_gpt_hist.shape[0]):
            corpus.append(
                "Paper "
                + self.ask_gpt_hist.iloc[i, 0]
                + " Cites Paper "
                + self.ask_gpt_hist.iloc[i, 1]
            )
        corpus = ", ".join(corpus)
        prompt = query + ":\n\n" + f"{corpus}\n"
        prompt = prompt[:char_limit]
    ...
    def ask_chatgpt_ngrams(
        self,
        char_limit=4097,
        api_key="your_api_key_here",
        query="give me insights about the following information relating the n-grams and their frequency",
        model="text-davinci-003",
        max_tokens=2000,
        n=1,
        temperature=0.8,
    ...
    def ask_chatgpt_sankey(
        self,
        char_limit=4097,
        api_key="your_api_key_here",
        query="give me insights about the following information from a network called Sankey",
        model="text-davinci-003",
        max_tokens=2000,
        n=1,
        temperature=0.8,
    ...
    def ask_chatgpt_wordcloud(
        ...
        flag = 0
        os.environ["OPENAI_KEY"] = api_key
        corpus = pd.DataFrame.from_dict(
            self.ask_gpt_wd, orient="index", columns=["Frequency"]
        )
